...
for(const [search,expected] of [
  ['?v=1620','home'],['?mode=worldcup&v=1620','worldcup'],['?mode=global&v=1620','worldcup'],
  ['?mode=news&v=1620','news'],['?mode=gaindes&v=1620','gaindes'],['?mode=final&v=1620','final'],['?team=spain&v=1620','team']
]){
...
code = code.replace(/\}\)\(\);\s*$/, 'window.__QG_V1620_TEST__={renderHome,renderWorldCupMemory,renderNewsHub,worldNewsHtml,renderGaindesHub,renderFinal,sortNewsNewestFirst,run,safeRun};})();');
const classes=classList();
...
const context={
  ...
  location:{search:'?v=1620'},history:{replaceState(){}},
...
context.__QG_V1620_TEST__.renderHome(selector,teams,{},[older,...newsStatic,dynamic],results,{});
...
if(!selector.innerHTML.includes('assets/home/espagne-championne-2026.png')) throw new Error('Lot A: nouveau visuel Espagne absent de la carte Mémoire');
...
selector.innerHTML='';
context.__QG_V1620_TEST__.renderNewsHub(selector,[dynamic,older],'gaindes');
if(!selector.innerHTML.includes('Les Brèves des Gaïndés')) throw new Error('Lot A: hub Brèves des Gaïndés absent');
if(selector.innerHTML.includes('Mondial Pulse Editorial')||selector.innerHTML.includes('Analyse, histoires fortes et signaux faibles')) throw new Error('Lot A: en-tête Brèves du Mondial encore injecté dans les Brèves des Gaïndés');
if(selector.innerHTML.includes('class="world-news-head"')) throw new Error('Lot A: doublon éditorial encore présent au-dessus des articles Gaïndés');

selector.innerHTML='';
context.location.search='?mode=worldcup&v=1620';
context.__QG_V1620_TEST__.renderWorldCupMemory(selector,teams,{},[],results,{});
...
context.location.search='?mode=worldcup&v=1620';
await context.__QG_V1620_TEST__.run();
...
const css=read('assets/css/app-v1620.css');
...
if(!css.includes('img.qg16-worldcup-poster{object-fit:contain')) throw new Error('Lot A: poster Espagne non protégé contre le recadrage');
if(!index.includes('<title>Passion Foot - Suivi des Lions</title>')) throw new Error('Lot A: nouveau nom de l’application absent');
if(!index.includes('og:title" content="Passion foot - suivi des gaïndés"')) throw new Error('Lot A: titre Facebook incorrect');
if(!index.includes('og:site_name" content="Passion foot - suivi des gaïndés"')) throw new Error('Lot A: nom de site Facebook incorrect');
if(index.includes('Mondial Pulse 2026')) throw new Error('Lot A: ancien nom encore présent dans index.html');
if(code.includes('Mondial Pulse 2026')) throw new Error('Lot A: ancien nom encore présent dans le renderer actif');
...
if(!admin.includes('id="cropCanvas"')||!admin.includes('id="cropZoom"')) throw new Error('Lot A: interface de recadrage absente');
if(!admin.includes("canvasBlob('image/jpeg',.86)")) throw new Error('Lot A: compression JPEG absente');
const adminScript=admin.match(/<script>([\s\S]*?)<\/script>/)?.[1];
if(!adminScript) throw new Error('Lot A: script de console introuvable');
new vm.Script(adminScript);
console.log('PASS Lot A V16.2.0 — marque, Facebook, visuel, Brèves Gaïndés et console photo');
