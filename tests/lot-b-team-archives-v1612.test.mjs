...
const code=fs.readFileSync(path.join(root,'assets/js/team-page-v1612.js'),'utf8');
...
async function render(team,lang){
  ...
  const context={console,URLSearchParams,location:{search:`?team=${team}&lang=${lang}&v=1612`},document,CustomEvent:class{constructor(type,options){this.type=type;this.detail=options?.detail}},fetch:async(url)=>({ok:true,json:async()=>readJson(url)}),setTimeout,clearTimeout};
...
if(archive.version!=='16.1.2'||archive.frozen!==true) throw new Error('Archive figée V16.1.2 absente');
...
if(/simulat|pronostic|choisis le vainqueur|champion simulé/i.test(code)) throw new Error('Vocabulaire de simulation présent dans le renderer V16.1.2');
...
if(!index.includes('assets/js/team-page-v1612.js?v=1612')) throw new Error('Renderer V16.1.2 non chargé');
if(!index.includes('assets/css/team-page-v1612.css?v=1612')) throw new Error('CSS V16.1.2 non chargé');
console.log(`PASS Lot B V16.1.2 — ${teams.length*langs.length} pages, récit avant trois archives figées identiques`);
