...
const code=fs.readFileSync(path.join(root,'assets/js/team-page-v1610.js'),'utf8');
...
async function render(team,lang){
  ...
  const context={console,URLSearchParams,location:{search:`?team=${team}&lang=${lang}&v=1610`},document,CustomEvent:class{constructor(type,options){this.type=type;this.detail=options?.detail}},fetch:async(url)=>({ok:true,json:async()=>readJson(url)}),setTimeout,clearTimeout};
...
for(const team of teams){
  for(const lang of langs){
    ...
    if(!html.includes('qg-v1610-team-root')) throw new Error(`${team}/${lang}: root missing`);
    ...
    if(!html.includes('/?v=1610')) throw new Error(`${team}/${lang}: home navigation missing`);
...
if(!index.includes('assets/js/team-page-v1610.js?v=1610')) throw new Error('Lot B: new renderer not loaded');
